...
test('renders properly', t => {
  ...
  t.is(wrapper.find('Header').prop('toggleAddModa'), wrapper.instance().toggleAddModaSection);
...
test('calling toggleAddModaSection dispatches toggleAddModa', t => {
  const wrapper = shallow(
    <App {...props} />
  );

  wrapper.instance().toggleAddModaSection();
  t.truthy(dispatch.calledOnce);
  t.truthy(dispatch.calledWith(toggleAddModa()));
});
